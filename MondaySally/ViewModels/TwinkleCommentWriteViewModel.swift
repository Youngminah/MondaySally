//
//  TwinkleCommentWriteViewModel.swift
//  MondaySally
//
//  Created by meng on 2021/07/21.
//

class TwinkleCommentWriteViewModel {
    ...
    func fetchTwinkleCommentWrite(with index: Int, with content: String){
        self.isLoading = true
        self.dataService?.requestFetchTwinkleCommentWrite(with: index, with: content, completion: { [weak self] response, error in
            if let error = error {
                self?.error = error
