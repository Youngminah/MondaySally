//
//  TwinkleWriteViewModel.swift
//  MondaySally
//
//  Created by meng on 2021/07/19.
//

class TwinkleWriteViewModel {
    ...
    func fetchTwinkleWrite(with input: TwinkleWriteInput){
        self.isLoading = true
        self.dataService?.requestFetchTwinkleWrite(with: input, completion: { [weak self] response, error in
            if let error = error {
                self?.error = error
